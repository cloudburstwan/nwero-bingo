...
export function TemplatesFreeSpacesAPI(database: Database, sessions: Sessions) {
  ...
  api.delete("/:id", requireSessionMiddleware(sessions), async (req: RequestWithSession, res) => {
    try {
      ...
      await database.templates.deleteFreeSpace(freeSpace);

      await database.addHistory(req.session!.userId, "templates_free_spaces", HistoryAction.DELETE, freeSpace.id, null);
